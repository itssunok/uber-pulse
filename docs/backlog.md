...
- `[done]` Mock data (`PIPELINES`, `CATALOG`, `ROADMAP_BASE`, `METRICS_BASE`) moved out of `Uber Pulse.dc.html` into `data/pipelines.js`, `data/catalog.js`, `data/roadmap.js`, `data/metrics.js` — plain JS globals, no build step, loaded via `<script src>` before `support.js`. Each file has a JSDoc header documenting field shape/types. Values preserved exactly, no new fields added.
...
- `[open]` Organize project folder structure — rename `Uber Pulse.dc.html` (odd filename, spaces + a stray `.dc` segment) to something conventional like `index.html`; move `favicon.png` and any other loose image assets into an `assets/` or `images/` folder instead of sitting at repo root. Cosmetic/hygiene, not business-critical, but cheap and makes the repo read as intentionally organized rather than assembled. Small effort.
