...
const METRICS_BASE = [
  ...
  { id:'cancellation_rate', label:'Trip Cancellation Rate', value:'8.9%', delta:'+0.2pp WoW', positive:false, points:[8.05,8.15,8.24,8.34,8.47,8.63,8.76,8.92], iconBg:'#FBE3E0', iconColor:'#A93327', featuredOnHome:false,
    tooltip:'Share of Ridesharing trip requests cancelled by either the rider or the driver before completion, in the trailing 7 days — (trip_cancelled_by_rider + trip_cancelled_by_driver) ÷ (completed_trip + those two events).', sourceEventId:'trip_cancelled_by_rider',
    ...
    segmentStats:[{label:'Rider-initiated', value:'6.4%'}, {label:'Driver-initiated', value:'2.5%'}],
    changelog:[
      {version:'v1.0', date:'2026-01-15', change:'Initial release, tracking cancellations by initiator for Ridesharing trip requests.'}] },
  { id:'on_time_pickup_rate', label:'On-Time Pickup Rate', value:'97.6%', delta:'-0.1pp WoW', positive:false, points:[98.7,98.5,98.3,98.2,98.0,97.9,97.7,97.6], iconBg:'#E6E9FB', iconColor:'#3E4FA6', featuredOnHome:false,
    tooltip:'Complement of the ETA-miss rate — the share of Ridesharing trips where the driver’s actual arrival was within 5 minutes of the last-calculated ETA, in the trailing 7 days. Declining alongside the ETA & Routing Events pipeline’s current degradation.', sourceEventId:'route_eta_missed_threshold',
    implementingTeam:'Rides Platform Engineering', definitionOwner:'Rides Analytics', downstreamConsumers:['Rides Growth Engineering', 'Reliability Engineering', 'Executive Dashboards'],
    segmentStats:[{label:'UberX', value:'98.1%'}, {label:'Comfort', value:'97.4%'}, {label:'Black', value:'98.6%'}, {label:'Pool/Share', value:'95.2%'}],
    changelog:[
      {version:'v1.0', date:'2026-01-15', change:'Initial release, measured as the complement of route_eta_missed_threshold volume.'}] },
